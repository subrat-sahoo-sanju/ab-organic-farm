...
@if($banners->count())
<section class="pb-4">
  <div class="mx-auto grid w-full max-w-7xl grid-cols-1 gap-4 px-4 sm:grid-cols-2 sm:px-6 lg:px-8">
      ...
      <a href="{{ $banner->button_url ?: '#' }}" class="group relative block overflow-hidden rounded-2xl bg-cream-100 shadow-sm transition duration-300 hover:shadow-xl">
        ...
        @if(!empty($banner->show_text))
          ...
          <div class="absolute inset-x-0 bottom-0 p-5 sm:p-6">
            @if($banner->subtitle)<span class="mb-1 inline-block rounded-full bg-[#74C9A1]/90 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">{{ $banner->subtitle }}</span>@endif
            <h3 class="font-display text-lg font-extrabold text-white sm:text-xl">{{ $banner->title }}</h3>
            @if($banner->button_text)
              <span class="mt-1 inline-flex items-center gap-1 text-sm font-bold text-[#74C9A1] transition group-hover:text-white">{{ $banner->button_text }}<x-lucide-arrow-right class="h-4 w-4" /></span>
            @endif
          </div>
        @endif
      </a>
