@php
    // Reference "Native Ingredients. No Substitutes." — gold title over full-width
    ...
    $titleColor = $cfg['title_color'] ?? '#B5762A';
    $bgDesktop  = $cfg['bg_desktop'] ?? null;
    $bgMobile   = $cfg['bg_mobile'] ?? $bgDesktop;
    $heading    = $sec->title ?: 'Native Ingredients. No Substitutes.';
@endphp
            ...
            <div class="overflow-hidden rounded-[8px]">
                <div class="flex items-center gap-3 transition-transform duration-500 ease-out"
                    ...
                    @foreach($items as $item)
                        ...
                        <a href="{{ ($item['url'] ?? '') ?: '#' }}" class="shrink-0" style="min-width: 0">
